[[Home]] > [[User Guide|UserGuide]] > [[Configuration API|Configuration-API]] > [[Query Processors|Configuration-Query-Processors]]
...
Alternatively you may use the `dnr:usingDataset` property to connect it to a Dataset instead. See [[Configuration API - Datasets|Configuration-SPARQL-Datasets]] for details. If both `dnr:usingDataset` and `dnr:usingStore` are present then `dnr:usingDataset` has priority and `dnr:usingStore` will be ignored.
...
The above specifies a Query Processor which passes the queries to the SPARQL engine of the specified Fuseki store. See [[Configuration API - Storage Providers|Configuration-Storage-Providers]] for more detail on configuring storage providers.
...
The Remote Query Processor is used to farm out queries to a remote SPARQL endpoint, it can be configured to use any endpoint configurable as described on [[Configuration API - SPARQL Endpoints|Configuration-SPARQL-Endpoints]].
